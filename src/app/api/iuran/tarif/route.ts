...
export async function GET() {
  try {
    const [rows] = await pool.execute<TarifRow[]>(
      `SELECT id, nominal_anggota, nominal_pengurus, periode_mulai, aktif, catatan, created_at, updated_at
       FROM iuran_tarif
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Number.isFinite(nominal_anggota) || nominal_anggota < 0) {
      return NextResponse.json({ error: "Nominal iuran anggota tidak valid" }, { status: 400 });
    }
    ...
    const [res] = await pool.execute<ResultSetHeader>(
      `INSERT INTO iuran_tarif (nominal_anggota, nominal_pengurus, periode_mulai, aktif, catatan)
       VALUES (?, ?, ?, ?, ?)`,
      [nominal_anggota, nominal_pengurus, normalized, aktif, catatan]
    );
