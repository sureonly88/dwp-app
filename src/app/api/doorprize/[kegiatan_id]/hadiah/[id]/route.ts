...
export async function POST(
  ...
  try {
    ...
    const picked = await pickRandomDoorprizeCandidate(kegiatan_id);
    if (!picked) {
      return NextResponse.json(
        { error: "Tidak ada peserta hadir yang tersedia" },
        { status: 400 }
      );
    }
    ...
    const [ins] = await pool.execute<ResultSetHeader>(
      ...
      [kegiatan_id, id, picked.peserta_tipe, picked.anggota_id, picked.tamu_id]
    );
    ...
    return NextResponse.json({
      ...
      winner: {
        ...
        peserta_tipe: picked.peserta_tipe,
        anggota_id: picked.anggota_id,
        tamu_id: picked.tamu_id,
        nama: picked.nama,
        nip: picked.nip,
        jabatan: picked.jabatan,
        unit_kerja: picked.unit_kerja,
        instansi: picked.instansi,
      },
    });
  } catch (err) {
    console.error(err);
