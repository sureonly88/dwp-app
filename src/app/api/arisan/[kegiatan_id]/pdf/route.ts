...
// GET /api/arisan/[kegiatan_id]/pdf
export async function GET(
  ...
  try {
    ...
    const [setupRows] = await pool.execute<SetupRow[]>(
      `SELECT nominal_per_orang, jumlah_pemenang
       FROM arisan_setup
       WHERE kegiatan_id = ?
       LIMIT 1`,
      [kegiatan_id]
    ...
    const [winnerRows] = await pool.execute<WinnerRow[]>(
      `SELECT w.id, w.urutan, w.waktu, a.nama, a.nip, a.jabatan, a.unit_kerja
       FROM arisan_winners w
       JOIN anggota a ON a.id = w.anggota_id
       WHERE w.kegiatan_id = ?
       ORDER BY w.urutan ASC, w.id ASC`,
      [kegiatan_id]
    );
    ...
    try {
      ...
      console.warn("Logo PDF arisan tidak dapat dimuat:", error);
    }
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="pemenang-arisan-${safeCode}.pdf"`,
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("PDF arisan error:", error);
    return NextResponse.json({ error: "Gagal membuat PDF daftar pemenang arisan" }, { status: 500 });
  }
}
...
function buildHtml(args: {
  ...
  const totalNominal = setup ? Number(setup.nominal_per_orang) * Number(setup.jumlah_pemenang) : 0;
  ...
  const rowsHtml = winners.length > 0
    ? winners.map((winner) => `
      <tr>
        ...
        <td>${escHtml(winner.unit_kerja ?? "-")}</td>
        <td class="center">${escHtml(formatTanggalJam(winner.waktu))}</td>
      </tr>
    `).join("")
    : `
      <tr>
        <td colspan="6" class="empty">Belum ada pemenang arisan.</td>
      </tr>
    `;
...
<head>
  <meta charset="UTF-8" />
  <title>Daftar Pemenang Arisan</title>
  <style>
  ...
  <div class="page">
    ...
    <div class="stats">
      <div class="stat">
        ...
        <div class="stat-value">${winners.length} orang</div>
      </div>
      <div class="stat">
        <div class="stat-label">Nominal per Orang</div>
        <div class="stat-value">${setup ? escHtml(formatRupiah(Number(setup.nominal_per_orang))) : "-"}</div>
      </div>
      <div class="stat">
        <div class="stat-label">Estimasi Total</div>
        <div class="stat-value">${setup ? escHtml(formatRupiah(totalNominal)) : "-"}</div>
      </div>
    </div>
